...
function selectRole(role) {
  ...
  const studentTypeSelection = document.getElementById('student-type-selection');
  const registerSection = document.getElementById('register-section');
  
  if (role === 'Student') {
    // If Student is selected, show student type selection
    roleSelection.classList.add('hidden');
    studentTypeSelection.classList.remove('hidden');
  } else {
    // For other roles, go directly to registration
    roleSelection.classList.add('hidden');
    ...
    // Show branch selection for teachers during registration
    const registerBranchSelection = document.getElementById('registerBranchSelection');
    const registerClassSelection = document.getElementById('registerClassSelection');
    
    if (role === 'Teacher') {
      registerBranchSelection.style.display = 'block';
      registerClassSelection.style.display = 'none';
    } else {
...
function selectStudentType(studentType) {
  ...
  studentTypeSelection.classList.add('hidden');
  registerSection.classList.remove('hidden');
  
  // Set the actual role based on student type
  const actualRole = studentType;
  document.getElementById('regRole').value = actualRole;
  document.getElementById('registerRoleText').textContent = studentType === 'SchoolStudent' ? 'School Student' : 'Engineering Student';
  
  // Show appropriate selection based on student type
  if (studentType === 'EngineeringStudent') {
    registerBranchSelection.style.display = 'block';
    registerClassSelection.style.display = 'none';
  } else if (studentType === 'SchoolStudent') {
    registerBranchSelection.style.display = 'none';
    registerClassSelection.style.display = 'block';
  }
}
...
function backToRoleSelection() {
  ...
  document.getElementById('loginSection').value = '';
}
...
function toggleForm(formType) {
  ...
  const registerBranchSelection = document.getElementById('registerBranchSelection');
  const loginClassSelection = document.getElementById('loginClassSelection');
  const registerClassSelection = document.getElementById('registerClassSelection');
  const studentTypeSelection = document.getElementById('student-type-selection');
  ...
  if (formType === 'login') {
    ...
    // Use the student type if it's set, otherwise use the selected role
    const effectiveRole = selectedStudentType || selectedRole;
    document.getElementById('loginRole').value = effectiveRole;
    document.getElementById('loginRoleText').textContent = effectiveRole === 'SchoolStudent' ? 'School Student' : 
      (effectiveRole === 'EngineeringStudent' ? 'Engineering Student' : effectiveRole);
    
    // Show branch selection for engineering students and teachers
    if (effectiveRole === 'EngineeringStudent' || effectiveRole === 'Teacher') {
      loginBranchSelection.style.display = 'block';
      loginClassSelection.style.display = 'none';
    } 
    // Show class selection only for school students
    else if (effectiveRole === 'SchoolStudent') {
      loginBranchSelection.style.display = 'none';
      loginClassSelection.style.display = 'block';
    } 
    ...
    // Use the student type if it's set, otherwise use the selected role
    const effectiveRole = selectedStudentType || selectedRole;
    document.getElementById('regRole').value = effectiveRole;
    document.getElementById('registerRoleText').textContent = effectiveRole === 'SchoolStudent' ? 'School Student' : 
      (effectiveRole === 'EngineeringStudent' ? 'Engineering Student' : effectiveRole);
...
function registerUser() {
  ...
  // Check branch selection for engineering students and teachers
  if ((role === 'EngineeringStudent' || role === 'Teacher') && !branch) {
    showNotification('Please select your branch', 'error');
    return;
  }
  
  // Check class selection for school students
  if (role === 'SchoolStudent' && !className) {
  ...
  const newUser = {
    ...
    password: password,
    role: role,
    branch: (role === 'EngineeringStudent') ? branch : null,
    className: role === 'SchoolStudent' ? className : null,
    section: role === 'SchoolStudent' && section ? section : null,
...
function loginUser() {
  ...
  // Check branch selection for engineering students
  if (role === 'EngineeringStudent' && !branch) {
    showNotification('Please select your branch', 'error');
    return;
  }
  ...
  if (user.role !== role) {
    showNotification(`Please login with correct role. You are registered as ${user.role}`, 'error');
    return;
  }

  // For engineering students, check branch match
  if ((role === 'EngineeringStudent') && user.branch && branch && user.branch !== branch) {
    showNotification('Please select the correct branch', 'error');
    return;
  }

  // For school students, check class match
  if (role === 'SchoolStudent' && user.className && className && user.className !== className) {
    showNotification('Please select the correct class', 'error');
    return;
  }
  
  // Update user's branch if engineering student and branch is selected
  if (user.role === 'EngineeringStudent' && branch) {
    user.branch = branch;
    db.setUsers(users);
  }
  
  // Update user's class/section if school student and class is selected
  if (user.role === 'SchoolStudent' && className) {
...
function initDashboard() {
  ...
  if (currentUser.role === 'SchoolStudent') {
    document.getElementById('userRole').textContent = `${currentUser.className}${currentUser.section ? ` (${currentUser.section})` : ''}`;
  } else {
  ...
  if (currentUser.role === 'SchoolStudent' || currentUser.role === 'EngineeringStudent') {
...
function loadAllCourses() {
  ...
  // Filter courses based on user's class and section
  let filteredCourses = courses;
  
  if (currentUser.role === 'SchoolStudent') {
    // For school students, filter by class and section
    filteredCourses = courses.filter(course => {
      // If course is for all classes, show it
      if (course.targetClass === 'all') return true;
      
      // If course target class matches user's class
      if (course.targetClass === currentUser.className) {
        // If course is for all sections, show it
        if (!course.targetSection) return true;
        
        // If course target section matches user's section
        if (course.targetSection === currentUser.section) return true;
      }
      
      return false;
    });
  } else if (currentUser.role === 'EngineeringStudent') {
    // For engineering students, filter by branch
    filteredCourses = courses.filter(course => {
      // If course is for all classes, show it
      if (course.targetClass === 'all') return true;
      
      // If course target class matches user's branch
      if (course.targetClass === currentUser.branch) return true;
  ...
  if (filteredCourses.length === 0) {
    coursesGrid.innerHTML = `
      <div class="empty-state">
        ...
        <p>${currentUser.role === 'Teacher' ? 'Create your first course!' : 'No courses available for your class/section.'}</p>
      </div>
    `;
    return;
  }
  ...
  filteredCourses.forEach(course => {
    ...
    courseCard.innerHTML = `
      ...
      ${course.targetClass !== 'all' ? `<div class="course-meta">
        <span class="course-target">🎯 ${course.targetClass}${course.targetSection ? ` (${course.targetSection})` : ''}</span>
      </div>` : ''}
      <div class="course-meta">
        <span class="course-duration">📅 ${course.duration}</span>
        <span class="course-teacher">👨‍🏫 ${teacher ? teacher.name : 'Unknown'}</span>
      </div>
      ${(currentUser.role === 'SchoolStudent' || currentUser.role === 'EngineeringStudent') && !isEnrolled && !isOwner ? 
...
function createCourse() {
  ...
  const targetClass = document.getElementById('courseTargetClass').value;
  const targetSection = document.getElementById('courseTargetSection').value;
  ...
  const newCourse = {
    ...
    targetSection: targetSection !== 'all' ? targetSection : null,
  ...
  const users = db.getUsers();
  const students = users.filter(u => u.role === 'SchoolStudent' || u.role === 'EngineeringStudent');
    ...
    <div style="margin-top: 30px;">
      ...
      ${courseAssignments.length > 0 ? courseAssignments.map(assignment => {
        ...
        <div class="assignment-item">
          ...
          ${(currentUser.role === 'SchoolStudent' || currentUser.role === 'EngineeringStudent') ? 
            mySubmission ?
...
function loadAssignments() {
  ...
  userAssignments.forEach(assignment => {
    ...
    assignmentCard.innerHTML = `
      ...
      ${(currentUser.role === 'SchoolStudent' || currentUser.role === 'EngineeringStudent') ? timeStatus : ''}
      ${assignment.videoUrl && (currentUser.role === 'SchoolStudent' || currentUser.role === 'EngineeringStudent') && !submission ? `
        <div style="background: #e3f2fd; padding: 12px; border-radius: 8px; margin: 10px 0; border-left: 4px solid #2196f3;">
...
function switchLeaderboard(type) {
  ...
  // If switching to students leaderboard, update the filter options
  if (type === 'students') {
    updateStudentLeaderboardFilters();
  } else if (type === 'faculty') {
    // If switching to faculty leaderboard, update the filter options
    updateFacultyLeaderboardFilters();
  } else {
    // Remove filters when switching away from both leaderboards
    const existingFilter = document.getElementById('leaderboardFilters');
    if (existingFilter) {
...
// Update faculty leaderboard with branch filters
function updateFacultyLeaderboardFilters() {
  const currentUser = db.getCurrentUser();
  
  // Only teachers can see the filter options
  if (currentUser.role === 'Teacher') {
    const filterContainer = document.createElement('div');
    filterContainer.id = 'leaderboardFilters';
    filterContainer.style.marginBottom = '20px';
    filterContainer.style.padding = '15px';
    filterContainer.style.background = '#f8f9fa';
    ...
    filterContainer.innerHTML = `
      <h4 style="margin: 0 15px 0 0; color: #764ba2;">Filter Faculty:</h4>
      <div>
        <label style="display: block; margin-bottom: 5px; color: #666; font-size: 13px;">Branch:</label>
        <select id="facultyBranchFilter" style="padding: 8px; border: 2px solid #e0e7ff; border-radius: 8px; font-size: 13px; background: white; cursor: pointer;">
      ...
      <button onclick="loadFacultyLeaderboard()" style="align-self: flex-end; padding: 8px 16px; background: #764ba2; color: white; border: none; border-radius: 8px; cursor: pointer; font-size: 13px;">Apply Filters</button>
    `;
    ...
    // Insert filter container before the faculty leaderboard
    const facultyLeaderboard = document.getElementById('facultyLeaderboard');
    if (facultyLeaderboard) {
      facultyLeaderboard.parentNode.insertBefore(filterContainer, facultyLeaderboard);
    }
  }
}

// Update student leaderboard with class/section filters
function updateStudentLeaderboardFilters() {
  const currentUser = db.getCurrentUser();
  ...
  if (currentUser.role === 'Teacher') {
    ...
    filterContainer.innerHTML = `
      <h4 style="margin: 0 15px 0 0; color: #667eea;">Filter Students:</h4>
      <div>
        <label style="display: block; margin-bottom: 5px; color: #666; font-size: 13px;">Class:</label>
        <select id="leaderboardClassFilter" style="padding: 8px; border: 2px solid #e0e7ff; border-radius: 8px; font-size: 13px; background: white; cursor: pointer;">
      ...
      <div>
        <label style="display: block; margin-bottom: 5px; color: #666; font-size: 13px;">Section:</label>
        <select id="leaderboardSectionFilter" style="padding: 8px; border: 2px solid #e0e7ff; border-radius: 8px; font-size: 13px; background: white; cursor: pointer;">
      ...
      <button onclick="loadStudentLeaderboard()" style="align-self: flex-end; padding: 8px 16px; background: #667eea; color: white; border: none; border-radius: 8px; cursor: pointer; font-size: 13px;">Apply Filters</button>
    `;
    ...
    // Insert filter container before the students leaderboard
    const studentsLeaderboard = document.getElementById('studentsLeaderboard');
    if (studentsLeaderboard) {
      studentsLeaderboard.parentNode.insertBefore(filterContainer, studentsLeaderboard);
    }
  }
}
...
function loadLeaderboards() {
  ...
  // Initialize filters for teachers
  if (currentUser.role === 'Teacher') {
    // Initialize student filters by default
    updateStudentLeaderboardFilters();
  }
...
function loadStudentLeaderboard() {
  const users = db.getUsers();
  let students = users.filter(u => u.role === 'SchoolStudent' || u.role === 'EngineeringStudent');
  const currentUser = db.getCurrentUser();
  
  // For teachers, apply class/section filters
  if (currentUser.role === 'Teacher') {
    const classFilter = document.getElementById('leaderboardClassFilter');
    const sectionFilter = document.getElementById('leaderboardSectionFilter');
    ...
    if (classFilter && sectionFilter) {
      ...
      if (selectedClass !== 'all') {
        students = students.filter(student => {
          if (student.role === 'SchoolStudent') {
            return student.className === selectedClass;
          } else if (student.role === 'EngineeringStudent') {
            return student.branch === selectedClass;
          }
          return false;
        });
      }
      ...
      if (selectedSection !== 'all') {
        students = students.filter(student => {
          ...
          // Engineering students don't have sections
          return student.role !== 'SchoolStudent';
        });
      }
    }
  }
  // For students, only show students from the same class/section
  else {
    if (currentUser.role === 'SchoolStudent') {
      // Show only students from the same class and section
      students = students.filter(student => 
        student.role === 'SchoolStudent' && 
        student.className === currentUser.className && 
        student.section === currentUser.section
      );
    } else if (currentUser.role === 'EngineeringStudent') {
      // Show only students from the same branch
      students = students.filter(student => 
        student.role === 'EngineeringStudent' && 
  ...
  if (students.length === 0) {
    document.getElementById('studentsLeaderboard').innerHTML = `
      <div class="empty-state">
        ...
        <p>${currentUser.role === 'Teacher' ? 'No students match the selected filters' : 'No students in your class/section'}</p>
      </div>
    `;
    return;
  }
...
function loadFacultyLeaderboard() {
  ...
  // For teachers, apply branch filters
  if (currentUser.role === 'Teacher') {
    ...
    if (branchFilter) {
      ...
      if (selectedBranch !== 'all') {
        teachers = teachers.filter(teacher => teacher.branch === selectedBranch);
      }
    }
  }
  ...
  if (teachers.length === 0) {
    document.getElementById('facultyLeaderboard').innerHTML = `
      <div class="empty-state">
        ...
        <p>${currentUser.role === 'Teacher' ? 'No faculty match the selected filters' : 'Leaderboard will appear once teachers create courses'}</p>
      </div>
    `;
    return;
  }
